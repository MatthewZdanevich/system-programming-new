...
int main() {
    ...
    if (function_choice == 1) {
        computation_thread = std::thread(writeFunctionData, function1, x_start, x_end, x_step, filename, std::ref(file_mutex));
    } else if (function_choice == 2) {
        computation_thread = std::thread(writeFunctionData, function2, x_start, x_end, x_step, filename, std::ref(file_mutex));
    } else {
