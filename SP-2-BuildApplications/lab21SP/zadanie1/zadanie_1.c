...
off_t pos;
size_t count;
/* Строки help_str, unkn_emsg, ifile_emsg, ofile_emsg, close_emsg и lseek_emsg служат для
вывода информационных сообщений. Как правило, это сообщения об ошибках */
...
case '?': write (2, help_str, ARR_SIZE (help_str));
return 1;
default:
...
if (ofname != NULL) { ofd = open (ofname, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP); /* 0640 */
