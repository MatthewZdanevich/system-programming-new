...
if (tfd == -1) { fprintf (stderr, "mkstemp() error\n"); exit (1); } 
unlink (tfile); 
printf ("Name: "); scanf ("%s", find_buffer); 
...
while (read (tfd, &ch, 1) > 0) write (fd_out, &ch, 1); 
close (fd_out); close (tfd); 
}
void abook_add (void) { 
printf ("Name: "); scanf ("%s", ab_entry[0].iov_base); 
printf ("Phone number: "); scanf ("%s", ab_entry[1].iov_base); 
printf ("E-mail: "); scanf ("%s", ab_entry[2].iov_base); 
int fd = open (ABOOK_FNAME, O_WRONLY | O_CREAT | O_APPEND, 
S_IRUSR | S_IWUSR | S_IRGRP); 
if (fd == -1) abook_failed (1); 
if (writev (fd, ab_entry, 3) <= 0) { fprintf (stderr, "Cannot write to address book\n"); 
exit (1); 
} 
close (fd); 
}
...
ab_entry[0].iov_base = name_buffer; ab_entry[0].iov_len = NAME_LENGTH; 
ab_entry[1].iov_base = phone_buffer; ab_entry[1].iov_len = PHONE_LENGTH; 
ab_entry[2].iov_base = email_buffer; ab_entry[2].iov_len = EMAIL_LENGTH; 
if (!strcmp (argv[1], "add")) { abook_add (); } 
else if (!strcmp (argv[1], "find")) { abook_find (); } 
else if (!strcmp (argv[1], "delete")) { abook_delete ();} 
